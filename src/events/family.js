...
export const FAMILY_EVENTS = [
  {
    ...
    choices: [
      ...
      },
      {
        label: '老實說今天可能得先走。',
        effects: { self: -3 },
        // 這個遊戲的配偶線是固定異性（男配郁涵、女配宗翰），相親對象也照那個設定走。
        // 原本這句寫死「她」，女性主角會被姑姑安排一場同性相親，
        // 而遊戲從來沒問過性向，別的地方也不支援——那會讀成一個意外，不是一個設定。
        log: (s) =>
          s.gender === 'f'
            ? '你回醫院處理了一件半小時就結束的事。姑姑隔天打來：「人家說你人很好，但他想找可以一起吃飯的。」'
            : '你回醫院處理了一件半小時就結束的事。姑姑隔天打來：「人家說你人很好，但她想找可以一起吃飯的。」',
      },
    ],
  },
  {
  ...
  },
  {
    ...
    text: (s) =>
      getStage(s).key === 'attending' || getStage(s).key === 'aesthetic'
        ? '婚宴訂了三次，延了三次：第一次是你接下的那台大刀，第二次是評鑑，第三次是科裡同時走了兩個人。飯店的訂金已經轉了兩次期。'
        : '婚宴訂了三次，延了三次：第一次是總醫師的班表，第二次是評鑑，第三次是科裡同時走了兩個人。飯店的訂金已經轉了兩次期。',
    choices: [
      {
        label: '這次無論如何都辦。',
