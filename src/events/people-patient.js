...
export const PATIENT_EVENTS = [
  // ───────── 第 0 幕：那台刀 ─────────
  {
    ...
    text: '凌晨兩點，急診推上來一個計程車司機，肚子硬得像一塊板子，血壓量不到。他站不起來，你們讓他左側躺著拍——肝臟和腹壁之間浮著一條氣。護理師唸出名字：王慶昌，五十二歲。',
    choices: [
      {
        label: '不等了，直接推刀房。',
  ...
  // ───────── 第 1 幕：他每年回來 ─────────
  {
    ...
    weight: 3,
    cond: after(2),
    text: '門診叫到王慶昌。他提著一袋自己種的芭樂進來，先把袋子放到你桌上才坐下：「醫師，今年這批比較甜。」胃藥他早就自己停了，傷口那條疤淡到快看不見。',
    effects: { self: 5 },
    bond: { patient: 4 },
  ...
  },
  {
    ...
    weight: 2,
    cond: after(4),
    text: '你在批價櫃檯前面聽見王慶昌跟旁邊的陌生人說：「這間醫院那個某某醫師，我的命是{他}救的。」他講得很大聲，那個陌生人一直點頭。',
    effects: { self: 4 },
    bond: { patient: 3 },
  ...
  // ───────── 第 2 幕：這一次你可能救不回來 ─────────
  {
    ...
    // 穿孔修補後不會年年照 CT。這一幕要由新症狀帶出來，才有理由安排影像。
    text: '這次他是自己說的：眼白有點黃，尿的顏色深，半年瘦了六公斤。他講得很輕鬆，你聽完就開了電腦斷層。片子出來，胰臟頭部兩公分，邊界不清楚。',
    effects: { self: -6 },
    bond: { patient: 4 },
    set: (s) => {
  ...
  },
  {
    ...
    stages: ['attending'],
    once: true,
    // 走抽籤的時候，過世的 53 局裡只有 23 局收得到這個盒子。而這是整條十二幕
    // 唯一讓他自己的聲音收尾的一幕——他走那幕演的是兒子的話與六顆芭樂，
    // 記事簿上那句「今仔日我又多活一年」才是這條線的量詞。七成的人收不到，
    // 等於多數結局裡病人自己沒有說到話。而且盒子是兒子送來的，不是機緣。
    // 比照 m_last_clinic：過世的隔年 forced。
    // （w_grandson 維持抽籤——孫子剛好輪到你的醫院見習本來就是運氣。）
    forced: (s) => !W(s).alive && s.age - (W(s).diedAt ?? -99) === 1,
