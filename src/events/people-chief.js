...
export const CHIEF_EVENTS = [
  // ───────── 第 0 幕：他要你的 paper、你的績效、你的時間 ─────────
  {
    ...
    weight: 3,
    cond: (s) => C(s).stage === 0,
    // 原本正文寫死「歸類計分零」而 HUD 上是 61，選項又寫死「我一年開四百台刀」，
    // 而這一局臨床只有 20%。兩個數字都要照實帶進來——
    // 而且不能只修正文漏掉按鈕，那是我上一輪犯過的錯。
    text: (s) =>
      `黃振邦把你的年度考核推過來，紅筆圈住一欄：「臨床沒問題。歸類計分 ${Math.round(s.attrs.papers)}。」他停了一下，「我不管你怎麼弄，明年這一格要好看一點。」`,
    choices: [
      {
        label: (s) => `「我開了 ${s.stats.surgeries} 台刀。」`,
        effects: { self: -3 },
        bond: { chief: -4 },
        log: (s) =>
          `「${s.stats.surgeries} 台不會出現在任何一張表上。」他把考核表收回去，「我知道這不合理。我也還是要跟你要。」`,
      },
      {
        label: '「我生得出來。」',
