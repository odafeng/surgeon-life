...
export const RESIDENT_EVENTS = [
    ...
    choices: [
      {
        ...
        effects: { self: 3 },
        log: (s) =>
          `你選了 A。那季點值 ${(s.pointValue ?? 0.82).toFixed(2)}，這台五個半小時的刀，醫院實拿一萬四千七。你的薪水跟這個數字沒有關係——今年還沒有。`,
      },
      {
        label: '把紀錄寫得完整一點。',
  ...
  // ── 論文與掛名 ──
  {
    ...
    choices: [
      ...
      {
        label: '爭取第一作者。',
        // 後果是研究合作與科內政治，這個遊戲沒有建那條線。
        // 拿升等用的教學服務分數來頂是錯的軸——跟 ac_be_reviewer 同一個毛病。
        // 沒有建模的東西就讓文字承擔。
        effects: { papers: 22, self: 3 },
        log: '主治愣了一下，說：「也可以啦。」你如願掛了第一。接下來一年，科裡的新計畫沒有再找過你——沒有人說為什麼，也不需要說。',
      },
    ],
  },
  {
