...
export const FAMILY_ARC_EVENTS = [
  // ───────── 相識 ─────────
  {
    ...
    text: '朋友介紹你認識了{配偶}。第一次見面，{她}問：「聽說你們醫師都很忙？」你發現自己已經三年沒有跟不是同事的人吃過飯。',
    choices: [
      {
        label: '認真試試看。',
        hint: '家庭 ≥ 15%',
        bond: { spouse: 10 },
        memory: '你對{配偶}說了實話：忙到沒有自己的時間。{她}笑了，說至少你誠實。',
        set: (s) => {
          s.family.stage = 'dating';
          s.family.floor = 15; // 剛好是關係能往前走的最低限度
          advance(s, 'spouse', 1);
        },
        log: '{她}笑了：「至少你誠實。」你們開始約會——大多約在醫院附近。從這個月起，你的行事曆上多了一格不能動的東西。',
      },
      {
        label: '現在真的沒空。',
        effects: { self: -2 },
  ...
  },
  {
    ...
    text: '你們的約會地點半徑不超過醫院八百公尺，因為你隨時可能被 call 回去。這個月第四次，你在{她}把主餐切開之前站起來。',
    effects: { self: -3 },
    bond: { spouse: 2 },
    log: '{她}把兩份都打包了。你回到刀房的時候，口袋裡還有那張沒用到的電影票。',
  },
  {
    ...
    text: '{配偶}把你的班表輸進了{她}自己的行事曆。{她}說這樣比較好安排，語氣像在講一件很普通的事。',
    effects: { familyBond: 8, self: 4 },
    bond: { spouse: 10 },
    memory: '{配偶}把你的班表輸進{她}自己的行事曆，說這樣比較好安排。',
    set: (s) => {
      s.family.stage = 'steady';
      advance(s, 'spouse', 2);
    },
    log: '愛一個外科醫師是一門需要天分的學問。{她}正在學，而且學得比你快。',
  },
    ...
    choices: [
      {
        ...
        log: '{她}哭著答應，只提了一個條件：「每週至少一起吃一頓晚餐。」你答應了。你們都知道這個承諾有多難——從今年起，家庭時間不得低於 20%。',
      },
      {
        ...
        bond: { spouse: -8 },
        log: '「等穩定一點」——這句話你已經說了三年。{她}笑了笑，把菜單遞給你。戒指又回到口袋裡。',
      },
    ],
  },
  {
    ...
    choices: [
      {
        ...
        log: '你按掉電話，繼續敬酒。後來聽說是學弟上的，病人活下來了。{配偶}一整晚都沒有提這件事。',
      },
      {
        label: '換下禮服，回醫院。',
    ...
    choices: [
      {
        ...
        log: '幾個月後的那個早上，餐桌上放著驗孕棒，兩條線。你盯著看了三秒，然後開始算預產期會落在哪一個月的班表上。算完才想起來要抱{她}一下——這個順序，你事後想了很久。',
      },
      {
        label: '「等生活穩定一點。」',
  ...
  },
  {
    ...
    text: (s) =>
      s.gender === 'f'
        ? '高層次超音波排在下午兩點。你早上還有一台，助理問要不要幫你把檢查改期——這個月你已經改過一次了。'
        : '高層次超音波，這個假你三個禮拜前就請了。臨時來了一台急刀，你打電話說：「你先進去，我盡量趕過去。」',
    choices: [
      {
        label: (s) => (s.gender === 'f' ? '不改。今天這個時間我要用。' : '把刀交給別人，趕過去。'),
        effects: { familyBond: 12, self: -3, clinical: -1 },
        bond: { spouse: 8 },
        log: (s) =>
          s.gender === 'f'
            ? '你脫了刷手服自己躺上檢查台，肚子上的耦合劑是冰的。技術師說：「手指頭都數得出來。」你看著螢幕，第一次覺得自己是病人。'
            : '你趕到時檢查剛結束。{她}把照片轉給你看：「醫師說手指頭都數得出來。」你盯著那張黑白的圖看了很久。',
      },
      {
        label: (s) => (s.gender === 'f' ? '改期。刀不能等。' : '開完刀再說。'),
        effects: { familyBond: -10, self: -5 },
        bond: { spouse: -8 },
        log: (s) =>
          s.gender === 'f'
            ? '改到三週後。那台刀開了五個小時，你站到最後，中間去廁所吐了一次沒有跟任何人講。'
            : '刀開了五個小時。你回撥時是晚上九點，{她}說：「沒關係，我有錄影。」那段影片你到現在都沒有看。',
      },
    ],
  },
  {
    ...
    text: (s) =>
      s.gender === 'f'
        ? '孩子出生那天，你在產檯上。麻醉科來的是你帶過的學弟，他認出你，愣了一下才開口：「老師，我要開始了。」'
        : '孩子出生那天，你在開刀。護理師站在無菌範圍外，把手機螢幕轉過來，隔著兩公尺給你看了照片。',
    effects: { familyBond: 8, self: 6 },
    bond: { spouse: 5 },
    memory: (s) =>
      s.gender === 'f'
        ? '孩子出生那天，替你麻醉的是你帶過的學弟。他叫你老師。'
        : '孩子出生那天你在開刀。護理師隔著兩公尺，把手機螢幕轉過來給你看。',
    set: (s) => {
      s.family.kids = 1;
    ...
    log: (s) =>
      s.gender === 'f'
        ? '你認得天花板那盞燈的型號。你聽見自己的孩子哭，然後才允許自己哭。躺在那裡的三十分鐘，是你這十年最長的三十分鐘。'
        : '你隔著口罩笑了，眼睛有點酸。然後你退開兩步，讓學弟接手收尾——手在抖的時候還硬要關腹，那不是敬業，是拿病人賭。',
  },
  {
    id: 'fa_first_steps',
    priority: true,
    ...
    log: '「你看到了。」{她}說。你才知道{她}已經替你錄了很多次，怕你錯過。',
  },
  {
    id: 'fa_kindergarten',
    priority: true,
  ...
  },
  {
    ...
    text: (s) =>
      s.gender === 'f' && s.age >= 40
        ? '{配偶}問：「要不要再生一個？」你算了一下自己的年紀，也算了一下如果要再請一次產假，會落在哪一年的升等送審。'
        : '{配偶}問：「要不要再生一個？」{她}說完自己先笑了：「我知道你會說等一下。」',
    choices: [
      {
        label: '這次不說等一下。',
        hint: '再一個孩子，存款每年多一份消耗',
        ...
        log: (s) =>
          s.gender === 'f'
            ? '第二次你什麼都知道了：知道要請哪幾天、知道回來刀表會怎麼排、知道那條走去哺乳室的最短路線。知道了並沒有比較輕鬆。'
            : '第二個孩子出生那天你在門診。你看完最後一個病人，脫了白袍就跑，趕上了。',
      },
      {
        label: '一個就好，我連一個都陪不了。',
        effects: { self: -3, familyBond: 2 },
        bond: { spouse: 2 },
        log: '{她}點點頭：「這樣也好。」你聽得出來那是真心的，也聽得出來那不是全部。',
      },
    ],
  },
  ...
  // ───────── 危機與修補 ─────────
  {
    ...
    forced: (s) =>
      ...
      !S(s).gone,
    text: '{配偶}把離婚協議書放在餐桌上，沒有情緒。「我不是要吵架。我只是想確認，這樣下去我們還算不算一家人。」',
    choices: [
      {
        ...
        bond: { spouse: 14 },
        memory: '{配偶}把離婚協議書放在桌上，你當場刪掉了明年三分之一的刀。',
        set: (s) => {
          s.family.neglect = 0;
      ...
      },
      {
        ...
        log: '{她}收起那張紙，說：「好。」你以為那是答應。三年後你才知道，那是{她}開始不再期待的那一天。從那年起，你們家的晚餐桌少了一個固定位置。',
      },
    ],
  },
  {
  ...
  },
  {
    ...
    text: '{配偶}的公司要外派{她}三年，是{她}等了很久的位子。{她}沒有問你的意見，只是說：「我推掉了。孩子總要有人顧。」',
    choices: [
      {
        label: '「你去。孩子我來想辦法。」',
      ...
      },
      {
        ...
        bond: { spouse: -8 },
        memory: '{配偶}推掉了外派，你什麼都沒說。',
        log: '{她}把那封信收進抽屜。很多年以後你整理東西翻到，才發現{她}連回覆信都寫好了，只是沒有寄。',
      },
    ],
  },
  {
    ...
    text: '你連值三天，回值班室發現桌上有一個便當。{配偶}送來的，你不在，{她}就放著走了。',
...

  {
    ...
    text: '你算了一下下半年的行程：升等送審、兩個計畫、加開的刀。怎麼排都排不出時間。{配偶}在客廳等你講話，{她}已經知道你要說什麼了。',
    choices: [
      {
        label: '「這一年真的不行，讓我拚完。」',
        hint: '承諾降為 10%',
        ...
        bond: { spouse: -10 },
        memory: '你跟{配偶}說，這一年真的不行，讓你拚完。{她}說好。',
        set: (s) => {
          s.family.floor = 10;
        },
        log: '{她}說：「好。」沒有吵，也沒有問是哪一年。那之後{她}開始自己安排週末，不再問你有沒有空。',
      },
      {
